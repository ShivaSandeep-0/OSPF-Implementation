...
public:
    ...
    void updateRoutingTable(std::set<Router*> all_router_ptrs) {
        ...
        for (auto& entry : routing_table) {
            ...
            if((neighbors_set.find(entry.second) != neighbors_set.end()) /*&& (neighbors_set.find(entry.first) == neighbors_set.end())*/)
            {
                if(routing_table[entry.second->getRouterID()] != entry.second)
                {
                    entry.second=routing_table[entry.second->getRouterID()];
                }
            }
        }
    }
...
// Modify the createRouter function to add both incoming and outgoing neighbors
